...
import { Container, Header, HeaderButton, HeaderAlert } from './styles';
import { Link } from 'react-router-dom';
import { openCart } from '../../store/actions/cart';
import { toggleSearch } from '../../store/actions/products';
...
const Topbar = () => {
  ...
  return (
    <Container>
      <Header>
        <Link to="/">
          <svg viewBox="0 0 512 88" preserveAspectRatio="xMidYMid">
        ...
        <div className="topbar__icons">
          <HeaderButton type="button" onClick={handleSearchClick}>
            <FiSearch size={20} color="#000" />
          </HeaderButton>
          <HeaderButton type="button" onClick={handleCartClick}>
            <FiShoppingBag size={20} color="#000" />
            {quantity > 0 && <HeaderAlert>{quantity}</HeaderAlert>}
          </HeaderButton>
        </div>
      </Header>
    </Container>
